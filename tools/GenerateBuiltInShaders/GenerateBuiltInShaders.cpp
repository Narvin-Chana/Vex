                                               ...
                                               const Span<const NonNullPtr<Shader>> spirvShaders)
{
    ...
    static auto OutputShaders = [](std::string& output, const Span<const NonNullPtr<Shader>>& shaders)
    {
        for (const NonNullPtr shader : shaders)
        {
            ...
            // Add constexpr key
            output += "static constexpr ShaderView MipGenerationCS =\n{\n";
...
int main()
{
    ...
    static const ShaderKey MipGenerationKey{
    ...
    auto shaderDxil = dxilSC.GetShader(MipGenerationKey);
    auto shaderSpirv = spirvSC.GetShader(MipGenerationKey);

    auto cppHeader = GenerateCppHeaderForShaders({ shaderDxil }, { shaderSpirv });

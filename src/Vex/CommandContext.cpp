                              ...
                              std::array<u32, 3> groupCount)
{
    ...
    std::vector<std::pair<ResourceBinding, RHITexture*>> rhiTextureBindings;
        ...
        [backend = backend, rhiTextureBindings, rhiBufferBindings](std::span<const ResourceBinding> resources) mutable
    {
        for (const auto& binding : resources)
        {
            if (binding.IsTexture())
            {
                ...
                rhiTextureBindings.emplace_back(binding, &texture);
            }
    ...
    CollectResources(reads);
    CollectResources(writes);

    // Transforms ResourceBinding into platform specific views, then binds them to the shader (bindlessly if possible).

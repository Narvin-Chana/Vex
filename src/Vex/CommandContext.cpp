    ...
    , cmdList(cmdList)
{
    ...
    // Inherit last known states for this queue.
    // !This means only one command context per queue should be open at a time to avoid sync issues!
    // Currently not enforced due to poor support for per-queue operations and queue resource ownership transfer not
    // being implemented.
    textureStates = graphics->textureStatesPerQueue[cmdList->GetType()];
}
                                  ...
                                  Span<TextureClearRect> clearRects)
{
    ...
    TextureSubresource aspectSubresource = binding.subresource;
    aspectSubresource.aspect = aspect;
...
{
    static auto IsWriteAccess = [](RHIBarrierAccess access) -> bool
    {
    ...
    TextureStateMap& textureStateMap = textureStates[texture.handle];
    ...
    textureStateMap.ForEachStateSection(texture.desc,
                                        ...
                                        [&](const TextureSubresource& section, RHITextureState srcState)
                                        {
                                            ...
                                            if (sameState && !IsWriteAccess(dstAccess))
                                            {
                                                return;
                                            }
...
void CommandContext::InferResourceBarriers(RHIBarrierSync syncStage, Span<const ResourceBinding> resources)
{
    for (const auto& rb : resources)
    {
        if (rb.IsBuffer())
        {
            ...
            RHIBarrierAccess dstAccess;
            switch (rb.GetBufferBinding().usage)
            {
            case ConstantBuffer:
            ...
            case RWByteAddressBuffer:
                ...
                break;
            default:
                VEX_LOG(Fatal, "Invalid usage for buffer binding {}...", rb.GetBufferBinding().buffer.desc.name);
                break;
            }
        ...
        else if (rb.IsTexture())
        {
            ...
            case ShaderReadWrite:
                ...
                break;
            default:
                VEX_LOG(Fatal, "Invalid usage for texture binding {}...", rb.GetTextureBinding().texture.desc.name);
                break;
            }

            EnqueueTextureBarrier(rb.GetTextureBinding().texture,
                                  rb.GetTextureBinding().subresource,
                                  syncStage,
                                  access,
                                  layout);
        }

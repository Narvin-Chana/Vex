...
struct _copyable_function
{
    struct lvalue_callable
    {
        ...
        void copy_into(std::byte* storage) const
        {
            copy_into_(storage);
        }
        void move_into(std::byte* storage) noexcept
        {
            move_into_(storage);
        }
    ...
    template <class Self>
    struct empty_object : lvalue_callable
    {
    ...
    class stored_object : constructible_lvalue
    {
    ...
    public:
        ...
        explicit stored_object(F&& f)
            requires(_is_not_self<F, stored_object> and not std::is_pointer_v<T>)
            : p_(std::make_unique<T>(std::forward<F>(f)))
        {
        }

        explicit stored_object(T p) noexcept
    ...
    public:
        explicit stored_object(T& target) noexcept
            : target_(target)
        {
        }
    ...
    public:
        template <class F>
        explicit target_object(F&& f) noexcept(std::is_nothrow_constructible_v<base, F>)
    ...
    class bound_target_object final : stored_object<T, bound_target_object<f, T>>
    {
        using base = stored_object<T, bound_target_object<f, T>>;

    public:
        template <class U>
        explicit bound_target_object(U&& obj) noexcept(std::is_nothrow_constructible_v<base, U>)
        ...
        R operator()(Args... args) const override
        {
            return std23::invoke_r<R>(f, this->get(), static_cast<Args>(args)...);
        }
    };
};
...
class function<S, R(Args...)>
{
    ...
    template <class F>
    using target_object_for = copyable_function::template target_object<std::unwrap_ref_decay_t<F>>;

    template <auto f>
    using unbound_target_object = copyable_function::template unbound_target_object<f>;

    template <auto f, class T>
    using bound_target_object_for = copyable_function::template bound_target_object<f, std::unwrap_ref_decay_t<T>>;

    template <class F, class FD = std::decay_t<F>>
    static bool constexpr is_viable_initializer = std::is_copy_constructible_v<FD> and std::is_constructible_v<FD, F>;

    alignas(typical_target_object) std::byte storage_[sizeof(typical_target_object)];

    auto storage_location() noexcept -> void*
    {
        return &storage_;
    }
    ...
    {
        return std::launder(reinterpret_cast<lvalue_callable const*>(&storage_));
    }
...
public:
    ...
    function() noexcept
    {
        ::new (storage_location()) empty_target_object;
    }
    ...
    template <class F>
    function(F&& f) noexcept(std::is_nothrow_constructible_v<target_object_for<F>, F>)
        requires _is_not_self<F, function> and is_invocable_using<lvalue<F>> and is_viable_initializer<F>
    {
    ...
    function(nontype_t<f>, U&& obj) noexcept(std::is_nothrow_constructible_v<bound_target_object_for<f, U>, U>)
        requires is_invocable_using<decltype(f), lvalue<U>> and is_viable_initializer<U>
    {
    ...
    {
        other.target()->copy_into(storage_);
    }
...
template <class S>
struct _strip_noexcept;

template <class R, class... Args>
struct _strip_noexcept<R(Args...)>
{
    using type = R(Args...);
};

template <class R, class... Args>
struct _strip_noexcept<R(Args...) noexcept>
{
    using type = R(Args...);
};

template <class S>
using _strip_noexcept_t = _strip_noexcept<S>::type;
...
template <class T>
function(T) -> function<_strip_noexcept_t<_drop_first_arg_to_invoke_t<decltype(&T::operator()), void>>>;

template <auto V>
function(nontype_t<V>) -> function<_strip_noexcept_t<_adapt_signature_t<decltype(V)>>>;

template <auto V, class T>
function(nontype_t<V>, T) -> function<_strip_noexcept_t<_drop_first_arg_to_invoke_t<decltype(V), T>>>;

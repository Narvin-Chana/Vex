...
template <class Sig>
struct _full_fn_sig
    : _ref_quals_fn_sig<Sig>
    , _noex_traits<false>
{
};

template <class R, class... Args>
struct _full_fn_sig<R(Args...) noexcept>
    : _ref_quals_fn_sig<R(Args...)>
    , _noex_traits<true>
{
};
...
struct _full_fn_sig<R(Args...) const noexcept>
    : _ref_quals_fn_sig<R(Args...) const>
    , _noex_traits<true>
{
};
...
template <class T>
constexpr auto _build_reference<std::reference_wrapper<T>> = [](auto& rhs) noexcept { return std::addressof(rhs); };
...
struct _move_only_pointer
{
    ...
    template <class T>
        requires std::is_object_v<T>
    constexpr explicit _move_only_pointer(T const* p) noexcept
        : val{ .cp_ = p }
    {
    }
...
struct _callable_trait
{
    ...
    static inline constinit vtable const callable_target{
        ...
        {
            if constexpr (not std::is_lvalue_reference_v<T> and not std::is_pointer_v<T>)
                delete get<T>(this_);
        },
    };
    ...
    static inline constinit vtable const boxed_callable_target{
        .call = [](handle this_, Args... args) noexcept(noex) -> R
        { return std23::invoke_r<R>(f, get<T>(this_), static_cast<Args>(args)...); },
        .destroy =
            [](handle this_) noexcept
        {
...
template <class T, template <class...> class Primary>
inline constexpr bool _does_not_specialize = not _is_specialization_of<std::remove_cvref_t<T>, Primary>;
...
class move_only_function<S, R(Args...)>
{
    ...
    template <class... T>
    static constexpr bool is_invocable_using = std::conditional_t<noex,
                                                                  std::is_nothrow_invocable_r<R, T..., Args...>,
...
public:
    ...
    move_only_function(F&& f) noexcept(std::is_nothrow_invocable_v<decltype(_take_reference), F>)
        requires _is_not_self<F, move_only_function> and _does_not_specialize<F, in_place_type_t> and
                 is_callable_from<VT> and std::is_constructible_v<VT, F>
    {
        ...
        vtbl_ = trait::template callable_target<std::unwrap_ref_decay_t<F>, inv_quals_f>;
    ...
    move_only_function(nontype_t<f>, T&& x) noexcept(std::is_nothrow_invocable_v<decltype(_take_reference), T>)
    ...
    move_only_function(nontype_t<f>, std::unique_ptr<T>&& x) noexcept
        ...
        : vtbl_(trait::template boxed_callable_target<f, T>)
        , obj_(x.release())
    {
    }
    ...
    explicit move_only_function(in_place_type_t<T>, Inits&&... inits) noexcept(
        ...
        : vtbl_(trait::template callable_target<std::unwrap_reference_t<T>, inv_quals_f>)
    ...
    explicit move_only_function(in_place_type_t<T>,
        ...
        std::is_nothrow_invocable_v<decltype(_build_reference<T>), decltype((ilist)), Inits...>)
    ...
    explicit move_only_function(nontype_t<f>,
        ...
        : vtbl_(trait::template bound_callable_target<f, std::unwrap_reference_t<T>, inv_quals_f>)
        , obj_(_build_reference<T>(std::forward<Inits>(inits)...))
    {
        static_assert(std::is_same_v<std::decay_t<T>, T>);
    }
    ...
    explicit move_only_function(nontype_t<f> t,
        ...
        requires std::is_base_of_v<C, T> and
                 is_callable_as_if_from<f, T*> and std::is_constructible_v<std::unique_ptr<T>, Inits...>
        : move_only_function(t, std::unique_ptr<T>(std::forward<Inits>(inits)...))
    {
    }

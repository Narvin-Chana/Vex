...
void VkCommandList::SetLayout(RHIResourceLayout& layout, RHIDescriptorPool& descriptorPool)
{
    if (layout.currentInternalConstantBuffer.has_value())
    {
        ...
        descriptorPool.UpdateBindlessMappingBuffer(*layout.currentInternalConstantBuffer);
    }
...
{
    std::array descriptorSets = { *descriptorPool.bindlessSet, *descriptorPool.bindlessMappingSet };
    switch (type)
    {
    case CommandQueueTypes::Graphics:
